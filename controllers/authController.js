...
/**
 * 
 * * This Funtion is used to set the role of the user if he is signing up with google or facebook
 * @param {Request} req
 * @param {Response} res
...
const setRole = async (req,res,next)=>{
  try{
    ...
    const user = await User.findByIdAndUpdate(_id,{role:role},{new:true}).select("-password -isDeleted -createdAt -updatedAt -__v");
    if(!user){
...
/**
 * 
 * This method is for both Freelancer and Buyer onboarding
 * @param {Request} req
 * @param {Response} res
 * @param {NextFunction} next
 * 
...
const onBoarding = async (req,res,next)=>{
  try{
    ...
    let { _id } = req.user;
    const response = await User.findOneAndUpdate({_id,onBoarding:false},{...req.body,onBoarding:true},{new:true,upsert:true}).select("-password -__v");
    
    if(!response){
      throw new notFound("User not found or have already onboarded");
    }
    return res.status(200).json({
      status: "OK",
      data: response,
